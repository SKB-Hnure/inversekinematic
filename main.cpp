...
int n[2][6]={{1,2,14,4,5,6},      //номера пинов для правой ноги
             {7,13,9,10,11,12}};    //для левой ноги
/*
...
else
{
 ...
 Q[3]=P[3]+dir[3]*(((Q[2]-P[2])*dir[2])-((Q[1]-P[1]))*dir[2]+g);
 }    //стопа вперед-назад (из микросекунд); здесь может быть ошибка!
...
{
  int i=0;
   cout<<"Номер ноги(правая - 1, левая - 2): ";
    cin>>i;
    cout<<"X="; cin>>x;
	cout<<"Y="; cin>>y;
	cout<<"Z="; cin>>dZ;
	cout<<"dY=";cin>>dY;
    leg_move(x,y,dZ,dY,L,P[i-1],I[i-1],n[i-1],dir[i-1]);
    cout<<endl;
   }
  return 0;
}
